...
\textbf{Time Complexity:} For each iteration, every vertex is assigned randomly, and the cut value is computed by checking all edges. If repeated $n$ times, the total complexity is $O(n(V+E))$. For a single run, it is $O(V+E)$.
...
\begin{verbatim}
Input: Graph G = (V, E)
u, w = endpoints of the heaviest edge in G
Initialize sets A = {u}, B = {w}
...
\textbf{Time Complexity:} Each vertex is considered once, and for each, the sum of edge weights to both sets is computed. Over all vertices, this totals $O(V+E)$.
...
\textbf{Time Complexity:} For each of $V$ assignments, all unassigned vertices are evaluated, and for each, edge sums are computed. This results in $O(V(V+E))$ overall.
...
\textbf{Time Complexity:} Each iteration checks all vertices and their neighbors, which is $O(V+E)$ per iteration. If $k$ is the number of improvement steps, total complexity is $O(k(V+E))$.
...
\textbf{Time Complexity:} For $n$ iterations, each performing semi-greedy construction ($O(V(V+E))$) and local search ($O(k(V+E))$), the total is $O(n(V(V+E) + k(V+E)))$. For practical purposes, this is often written as $O(n(V+E + k(V+E)))$.
\\
\\
\textbf{Note on Parameters:}
\\
\textbf{Alpha Parameter Tuning:}
The semi-greedy algorithm is tunable via the $\alpha$ (alpha) parameter, which controls the balance between greediness and randomness when selecting the next vertex to assign:

\begin{itemize}
\item \textbf{When $\alpha = 0$:} The threshold $\mu$ becomes the minimum possible value ($w_{\min}$). Only the most "greedy" candidates (those with the highest gain) are chosen, making the algorithm behave like a pure greedy algorithm.

\item \textbf{When $\alpha = 1$:} The threshold $\mu$ becomes the maximum possible value ($w_{\max}$). All candidates with the highest possible gain are included, but since the restricted candidate list can be large, the selection among them is random, making the algorithm behave more randomly.

\item \textbf{For $0 < \alpha < 1$:} The threshold $\mu$ is between $w_{\min}$ and $w_{\max}$. The restricted candidate list includes all candidates whose greedy value is at least $\mu$, so the algorithm mixes greedy selection with random choice among good candidates.
\end{itemize}

\textbf{Detailed Example:}
In the semi-greedy algorithm, for each unassigned vertex, you calculate a "greedy value" (how good it would be to assign it to a set). You find the minimum ($w_{\min}$) and maximum ($w_{\max}$) of these values, then set a threshold: $\mu = w_{\min} + \alpha \times (w_{\max} - w_{\min})$.

When $\alpha = 1$: $\mu = w_{\min} + 1 \times (w_{\max} - w_{\min}) = w_{\max}$. So, only vertices whose greedy value is at least $w_{\max}$ are considered for assignment. Usually, only the vertices with the highest possible greedy value are included in the restricted candidate list. If multiple vertices have this highest value, you pick one randomly from them.

This introduces randomness into the selection, even though you're only considering the best candidates. With $\alpha = 1$, you always pick from the best candidates, but if there are ties, you choose randomly among them, making the process less deterministic and more random than pure greedy.

\textbf{Summary:}
\begin{itemize}
\item $\alpha = 0$: Pure greedy (always picks the best candidate)
\item $\alpha = 1$: Pure random among the best candidates
\item $0 < \alpha < 1$: Mix of greedy and random, allowing you to tune the balance between exploitation (greedy) and exploration (random)
...
\begin{table}[h]
    ...
    \begin{tabular}{|lcccc|}
        ...
        \midrule
        Randomized & Low & Very Fast & No & $O(n(V+E))$ \\
        Greedy & Medium & Fast & Yes & $O(V+E)$ \\
        Semi-Greedy & Medium-High & Medium & No & $O(V(V+E))$ \\
        Local Search & High & Slow & Depends on initial & $O(k(V+E))$ \\
        GRASP & Very High & Slowest & No & $O(n(V(V+E) + k(V+E)))$ \\
        \bottomrule
    \end{tabular}
\end{table}
